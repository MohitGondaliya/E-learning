...
import CourseCard, { type FullCourseCard } from "../../../shared/component/CourseCard";
...
const CourseListing: React.FC = () => {
  ...
  const handleFilterTabChange = (_event: React.SyntheticEvent, newValue: number) => {
    setValueFilterTab(newValue);
  };

  // Rating Value
  const [ratingValue, setRatingValue] = React.useState<number[]>([2, 5]);
  const handleRatingValueChange = (_event: Event, newValue: number | number[]) => {
  ...
  const handlePriceValueChange = (_event: Event, newValue: number | number[]) => {
    if (Array.isArray(newValue)) {
  ...
  const handleVideoValueChange = (_event: Event, newValue: number | number[]) => {
    if (Array.isArray(newValue)) {
        ...
        <>
          ...
          <Grid container spacing={3}>
            {webDevelopmentCourses.map((course, index) => (
              <Grid size={{ xl: 3, md: 4, sm: 6, xs: 12 }} key={index}>

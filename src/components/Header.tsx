...
import { ExcludeRoutes, ROUTES } from "../shared/route";
...
const Header: React.FC = () => {
  ...
  return (
    <header className={`header ${isSidebarOpen ? 'open' : ''} `}>
      <Grid container spacing={2} justifyContent="space-between" alignItems="center" className="flex-grow-1">
        <Grid size={'auto'}>
          <Grid container spacing={1} alignItems="center">
            <Grid size={'grow'} className="d-lg-none d-block">
              <IconButton color="primary" size="large" onClick={toggleSidebar}>
                <Avatar src={images.MenuGrey} variant="square" alt="MenuGrey" />
              </IconButton>
              <Link to={ROUTES.Home} className="ms-sm-1 ms-4p logo" title="E-Learning">
                <img src={tabletScreen ? images.LogoColored : images.LogoColoredHorizontal} alt="E-Learning" />
              </Link>
            </Grid>
            <Grid size={'grow'} className="d-lg-block d-none">
